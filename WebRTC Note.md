...
（1）webrtc的拥塞控制算法为GCC算法，使用RTP/RTCP协议实现，并对两个协议做了扩展。
...
RTP协议可以检测分组丢失，检测出错顺序，并保持采样和播放之间的同步关系。
...
1、[降低网络拥塞，追求美好体验——对话拍乐云首席科学家章琦 (juejin.cn)](https://juejin.cn/post/6982067686679248904)

（1）成立于2019年，拍乐云是国内第一家视频会议背景的实时互动通信云服务提供商，汇聚了一大批专注于音频、视频、白板、网络、AI等领域的资深技术专家。

（2）拍乐云网络拥塞解决方案

带宽预测，动态码率调整和自适应的FEC（前向纠错）、ARQ（自动重传请求）、PLC（丢包隐藏）等技术，可以实现在极限场景下依然通话流畅。

拍乐云还自建了Pano Backbone全球实时传输加速网络，解决跨区域、跨国的链路问题，降低拥塞发生的概率，保障音视频的通信质量。

（3）所有抵抗弱网的手段都要付出代价，抗弱网会带来其他性能的受损。

传输上的丢包是随机事件，在数据发送的时候无法得知这个数据是否会在传输时被丢。前向纠错编码虽然具备抗丢包能力，但是它对于数据包的保护是比较盲目的，客观上导致传输效率降低。

（4）有些使用传统技术无法解决的问题，可以通过与AI技术的融合，大大降低问题解决的难度。无论是RTC行业的核心音视频编解码，还是其他一些音视频处理和增强技术，比如现在比较热的音频降噪、 视频超分、对象分割识别等，在AI的加持下，都获得了突破。

（5）音视频发展趋势

深度学习、虚拟现实技术、3D视频等等。虚拟现实和3D视频会依赖于相关硬件技术的成熟，我相信这一天不会太远了。

（6）音视频技术书推荐

Write Great Code Volume 2: Thinking Low-Level, Writing High-Level。

对音视频开发来说，代码执行的效率是非常重要的指标。

（7）现代编码器在 Rate-distortation 性能的提升上是以复杂度的上升为代价的，当前应用设备的碎片化非常严重、设备的运算能力差异巨大，这些都是新技术落地实时音视频系统将面临的挑战。在设计实时视频编码器时，应该同时考量复杂度和实时性的平衡。
